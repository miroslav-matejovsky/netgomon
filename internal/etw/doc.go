...
//
// Design Decisions:
//   - Implementations live in sub-packages (custom, rawsec, goetw) and are selected by the monitor.
//   - custom is primary (direct Win32 syscalls, no third-party deps), goetw/rawsec are fallbacks.
//     Only one backend runs per PID to avoid double-counting.
